...
def diagCallback(data):
 for i in data.status:
  ...
  if i.name == "husky_node: system_status":
   ...
   if override == False: battery_voltage(i.values[1].value)
...
rospy.Subscriber('/fake_fault', Float64 , fakeFaultCallback)
...
while not rospy.is_shutdown():
 ...
 t_trans[0] = 1# 1 meter
 for i in range(0,4): # 4 times - 0,1,2,3
  print("step..." + str(i+1))
